...
$json_string='{"sensorid":"605","date":"2014-9-12"}';
...
$dates=$obj -> date;
$dateList=array();
array_push($dateList,date("Y-m-d",strtotime($dates)));
	...
	*/
	...
	if($row['manual']==0){
		...
		if($startstation==1 || $startstation==2 || $founddata=0){
			...
			$sql="select stime from  basedata_" . $ydatesort . " where sensorid=$scode and detectedposition>2  and  stime<'22:00:00' order by stime desc limit 0,1";
			$result=mysql_query($sql,$conn); 
			if($row=mysql_fetch_array($result)){
		...
		if($founddata==0){
			...
			$sql="select detectedposition from  basedata_" . $tdatesort . " where sensorid=$scode  and stime>='07:00:00' and  stime<='12:00:00' and (detectedposition=5 or detectedposition=6)  order by stime limit 0,1";	
			if($row=mysql_fetch_array($result)){
