...
var txtLib=({
	...
	s_sync:{cn:"上次同步",en:"Last Sync"},
	s_battery:{cn:"电池电量",en:"Battery"},
	s_cal:{cn:"卡路里",en:"Calories"},
	s_distance:{cn:"行走距离",en:"Travel"},
	s_steps:{cn:"行走步数",en:"Steps"},
	s_sleep:{cn:"睡眠",en:"Sleep"},
	s_deepsleep:{cn:"深睡眠",en:"Deep sleep"},
	s_caltaken:{cn:"卡路里消耗量",en:"cal burned"},
	s_calleft:{cn:"未完成卡路里",en:"cal remain"},
	s_steptaken:{cn:"已完成步数",en:"Steps Taken"},
	s_status:{cn:"状态：",en:"Status:"},
	s_from:{cn:"开始：",en:"From:"},
	s_to:{cn:"结束：",en:"To:"},
	s_angle:{cn:"角度：",en:"Angle:"},
	s_nodata:{cn:"暂无.",en:"Not given."},
...
var periodList=Array("day","week","month");
var memberShowPeriod="day";
document.write("<script type='text/javascript' src='../js/reglogin.js'></"+"script>"); 
	...
	carTitleItem[1]={leftTitle:getText('s_cal'),iconImage:"act_bar_cal.png",currentValue:0,totalValue:0,percent:0,units:"Calories"};
	carTitleItem[2]={leftTitle:getText('s_steps'),iconImage:"act_bar_step.png",currentValue:0,totalValue:0,percent:0,units:"Steps"};
	carTitleItem[3]={leftTitle:getText('s_distance'),iconImage:"act_bar_dis.png",currentValue:0,totalValue:0,percent:0,units:"Km"};
...
$(function(){
	//if($.browser.msie){$.fx.off=true}
		...
		*/
		...
		//-------------------改变显示周期----
		$("img[id^='perPerioImg']").each(function(){
			$(this).css('cursor','pointer');
			$(this).click(function(){switchShowPeriod($(this).attr("id"));});
			
		});
		$("div[id^='perPerioTxt']").each(function(){
			$(this).css('cursor','pointer');
			$(this).click(function(){switchShowPeriod($(this).attr("id"));});
...
function showFamOut(event){
	...
	*/
	...
	function loadFamilyData(){
		showLoading('FAM');
		back_ecode=$.cookie('back_ecode');
		var outData={type:"family",lang:LANG,ucode:back_ucode,scode:back_scode,dates:dateStr,cdate:mytime,ecode:back_ecode,source:"w"};
		$.ajax({type: "POST",contentType: "application/json",dataType: "json",
			url:'../res/getpercentage.php',
			data:JSON.stringify(outData), 
        	success: function (msg) {
				if(msg.status !=200){
					$.cookie('back_ucode', null);
	...
	function dealActivityData(r){
		...
		for(var i=0;i<dataList.length;i++){
			...
			disDataList.push( subData[0] +"|" + Math.round(subData[3]/100,2)+ "|0");
...
function showSensor(){
	...
	if (nickname==""){
		nickname=getText('s_empty');
	}
	$('#INFO_name').html(nickname);
	...
	$('#INFO_sensorStation').html('<span>'+getText('s_sync')+':</span><br />'+getDateInfo(obj.lastupdate)+'<br /><span>'+getText('s_battery')+': </span>'+obj.power+'%');
...
barItem.prototype.init=function(){
	...
	switch(this.p.belongs){
		case "s_proBarAct":
			...
			this.leftDown=this.p.totalValue+" "+getText('s_cal');
			this.backPic="s_back.jpg";
			this.frontPic="s_f_act.jpg";
			break;
		case "s_proBarSlp":
			this.leftTitle=getText('s_sleep');
		...
		case "m_proStep":
			...
			this.leftDown=this.p.currentValue+getText('s_steptaken');
			this.backPic="l_back.jpg";
			this.frontPic="l_f_act.jpg";
			break;
		case "m_proMile":
			...
			this.leftDown=(this.p.currentValue).toFixed(3)+" "+$.cookie('back_unit')+" "+getText('s_distance');
			this.backPic="l_back.jpg";
			this.frontPic="l_f_act.jpg";
			break;
		case "l_proBarSlp":
			this.leftTitle=formatTime(this.p.currentValue);
			this.leftDown=getText('s_deepsleep');
...
function switchShowPercent(nid){
	...
	if(tempItemID==percentItemID){
	    ...
	    drawContextList[0].toValue=100*eval('familyAverange.'+memberShowPeriod+'.'+percentGroup[percentItemID])
		for(var i=0;i<drawContextList.length-1;i++){
			drawContextList[i+1].toValue=100*eval("familyList["+i+"].percentage."+memberShowPeriod+"."+percentGroup[percentItemID]);
...
function switchShowPeriod(nid){
	
	var tempItemID=parseInt(nid.substring(11,12));
	if(memberShowPeriod==periodList[tempItemID]){
		return;
	}else{
		memberShowPeriod=periodList[tempItemID];
		
		$("img[id^='perPerioImg']").attr("src","../images/se_no.png");
		$('#perPerioImg'+tempItemID).attr("src","../images/se_yes.png");
...
function drawDetail(){
	...
	drawContextList.push({canvas:canvas,buffer:canvasBuffer,fromValue:0,toValue:100*eval('familyAverange.'+memberShowPeriod+'.'+percentGroup[percentItemID])});
	
	for(var i=0;i<itemLength;i++){
		//alert(familyList[i].percentage.day.calories);
		//alert("familyList["+i+"].percentage."+memberShowPeriod+"."+percentGroup[percentItemID]);
		//alert(eval("familyList["+i+"].percentage."+memberShowPeriod+"."+percentGroup[percentItemID]));
	    ...
	    canvasBuffer.width = canvas.width;  
	    canvasBuffer.height = canvas.height; 
		drawContextList.push({canvas:canvas,buffer:canvasBuffer,fromValue:0,toValue:100*eval("familyList["+i+"].percentage."+memberShowPeriod+"."+percentGroup[percentItemID])});
		$('#aroundCircle'+i+' div').eq(0).css("color","white");
		$('#aroundCircle'+i+' div').eq(1).css("color","white");
		$('#aroundCircle'+i+' div').eq(2).css("color","#6a6a6a");
		var relation;
		if(i==0){
			relation=getText('s_me');
		}else{
			relation=familyList[i].relation
		}
		
		var nickname=familyList[i].nickname;
		if(nickname==''){
			nickname=getText('s_empty');
		}
		$('#aroundCircle'+i+' div').eq(2).text(nickname+" ("+relation+")");
		if(i>0){
