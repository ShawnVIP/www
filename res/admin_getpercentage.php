...
for($i=0;$i<count($memberList);$i++){
	...
	array_push($outdata,array('scode'=> $memberList[$i][scode],'relation'=>$memberList[$i][relation],'nickname'=>$memberList[$i][nickname],'headimage'=>$memberList[$i][headimage],'goallist'=>$memberList[$i][goalList][0],'percentage'=>$memberList[$i][percentage]));
...
echo json_encode(array('status'=>200,'memberlist'=>$outdata,'summary'=>$summary,'ecode'=>$ecode));
