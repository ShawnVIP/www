...
//$json_string='{"type":"act","ucode":"9rAFnWDjnvQDuRksRMpHnhWMrWbpf7ePJYtI","scode":"654","dates":"2014-8-29","cdate":"2014-8-29 下午3:22:41","ecode":"MgknIIKiVHhyjAjn","source":"w"}';
...
$sql="SELECT totime,position,lasttime FROM sensorstation where sensorid=$scode and sdate='$reqdate' and adjtype=0 order by totime";
$result=mysql_query($sql,$conn); 	


while ($row=mysql_fetch_array($result)){
	$stime=$row['totime'];
	$tmpdate=date("Y-m-d H:i:s",strtotime($dates . " " .$stime));
	for($i=0;$i<$row['lasttime'];$i++){
			$newtime=date('H:i:s',strtotime("$tmpdate -$i minute"));	
			$statusList[timeToRealID($newtime)]=$row['position'];
	}
...
for($j=2;$j<1440;$j++){
	if($statusList[$j] !=$statusList[$j-1]){
		array_push($edata, array('t'=>realIdToTime($j-1),'i'=>$statusList[$j-1]));	
		$lasttime=1;
	}else{
		$lasttime++;
	}
