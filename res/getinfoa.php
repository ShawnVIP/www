...
/*
create view totalinfo as select b.fallalert,b.positionalert, b.defaultgoal, b.transmitpower,b.fallthreshold,b.fallimpact,b.fallangleh,b.fallanglel,a.orderlist,a.userid,a.station,a.connected,a.power,a.lastuppower as lastupdate,b.nickname,b.headimage,b.dob,b.gender,b.unit, b.vipmode, c.*,b.detailid  from sensorlist as a, sensorinfo as b,dailyvalue as c where  a.sensorid=b.id and c.sensorid=b.id
*/


$json_string=$GLOBALS['HTTP_RAW_POST_DATA'];
//$json_string='{"ucode":"7ZYSquiG2Q0BEibjMXpYJnPnydPgtIdUCq9M","scode":"1","ecode":"CvYdlBkGHxgUfH3d","source":"w"}';
$json_string='{"ucode":"VN2AFQ4866XBnjNRDs40s2C8RaIQoTVCLP3K","scode":"419","ecode":"asL9rForZLW5tlOS","source":"w","cdate":"2014-4-18 09:44:51"}';
...
$sql ="SELECT orderlist, station, connected, power, lastupdate,nickname, headimage, dob, gender, unit, height, weight,fallalert,positionalert,defaultgoal, transmitpower,fallthreshold,fallimpact,fallangleh,fallanglel, step, stepgoal, stepwidth, distancegoal, runningwidth, bmi, sensorid, updated, age, bmr, caloriesgoal,sleepgoal, totalcal, totalsteps, totaldistance, totalsleep,detailid,vipmode FROM totalinfo  WHERE  userid=? and date=? order by orderlist";

echo "SELECT orderlist, station, connected, power, lastupdate,nickname, headimage, dob, gender, unit, height, weight,fallalert,positionalert,defaultgoal, transmitpower,fallthreshold,fallimpact,fallangleh,fallanglel, step, stepgoal, stepwidth, distancegoal, runningwidth, bmi, sensorid, updated, age, bmr, defaultgoal,sleepgoal, totalcal, totalsteps, totaldistance, totalsleep,detailid,vipmode FROM totalinfo  WHERE  userid='$ucode' and date='$now' order by orderlist";
...
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($orderlist, $station, $connected, $power,$lastupdate, $nickname, $headimage, $dob, $gender, $unit,  $height, $weight,$fallalert,$positionalert,$defaultgoal, $transmitpower,$fallthreshold,$fallimpact,$fallangleh,$fallanglel, $step, $stepgoal,  $stepwidth, $distancegoal, $runningwidth, $bmi, $sensorid, $updated, $age, $bmr, $caloriesgoal,$sleepgoal, $totalcal, $totalsteps, $totaldistance, $totalsleep,$detailid,$vipmode); 
while ($stmt->fetch()) {
	...
	array_push($value,$defaultgoal);
	array_push($vname,"fallalert");
	array_push($value,$fallalert);
	array_push($vname,"positionalert");
	array_push($value,$positionalert);
	array_push($vname,"para0");
	array_push($value,$transmitpower);
	array_push($vname,"para1");
	array_push($value,$fallthreshold);
	array_push($vname,"para2");
	array_push($value,$fallimpact);
	array_push($vname,"para3");
	array_push($value,$fallangleh);
	array_push($vname,"para4");
	array_push($value,$fallanglel);
