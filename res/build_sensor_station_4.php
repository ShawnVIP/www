...
function buildSensorStation($scode,$dateList){
	...
	for($i=0;$i<count($dateList);$i++){
		...
		//echo $sql;
		$result=mysql_query($sql,$conn); 
		$fromtime=date("Y-m-d H:i:s",strtotime($ldate . " 00:00:00"));
		$totime==date("Y-m-d H:i:s",strtotime($ldate . " 00:00:00"));;
		...
		while($row=mysql_fetch_array($result)){
			...
			$nowtime=date("Y-m-d H:i:s",strtotime($row['stime']));
			if($nowtime<$fromtime){$fromtime=$nowtime;}
			if($nowtime>$totime){$totime=$nowtime;}
