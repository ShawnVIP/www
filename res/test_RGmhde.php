	...
	for($i=0;$i<count($dateList);$i++){
		...
		while ($row=mysql_fetch_array($result)){
			...
			for($k=0;$k>-5;$k--){
				...
				if($k==0 && $detectedposition==1){
					array_push($checkList, timeToRealID($newtime)-4);
				}
			}
		...
		//----------------处理单独1的问题，单独1，前或者不是2，换成7
		for($j=0;$j<count($checkList);$j++){
			$independ=1;
			if($j>0 && $statusList[$checkList[$j]-5]==1){
				$independ=0;
			}
			if($j<count($checkList)-1 && $statusList[$checkList[$j]+5]==1){
				$independ=0;
			}
			if($independ==1){
				for($k=0;$k<5;$k++){
					$statusList[$checkList[$j]+$k]=7;
				}
