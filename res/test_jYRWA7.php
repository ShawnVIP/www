...
$json_string='{"sensorid":"605","date":"2014-9-11"}';
	...
	for($i=0;$i<count($dateList);$i++){
		...
		for($i=0;$i<count($ordList);$i++){
			...
			if($ordList[$i][position]==1 && $ordList[$i][lasttime]==5){
				if($i==0 && $ordList[1][position] !=1){
					$independ=1;
				}elseif($i=count($ordList)-1 && $ordList[$i-1][position] !=1){
					$independ=1;
				}elseif($ordList[$i-1][position] !=1 && $ordList[$i+1][position] !=1){
					$independ=1;
				}
