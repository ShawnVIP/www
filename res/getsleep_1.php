...
while($row=mysql_fetch_array($result)){
	...
	array_push($moveList,  timeToRealID($row['stime']) ."|" . $row['move']."|".$row['detectedposition']."|0");
		//array_push($moveList,  timeToRealID($stime)-720 ."|" . $stime ."|". $move);
}
...
while($row=mysql_fetch_array($result)){
	...
	array_push($moveList,  timeToRealID($row['stime'])-1440 ."|" . $row['move']."|".$row['detectedposition']."|1");
		//array_push($moveList,  timeToRealID($stime)-720 ."|" . $stime ."|". $move);
}
